...
- 🎤 **GPT-SoVITS 语音**：集成 GPT-SoVITS v2 Pro Plus，支持多音色语音合成
- 💬 **LLM 对话**：支持 OpenAI / Claude / 其他兼容 API
- 🧠 **长期记忆**：基于 Mem0 的持久化记忆系统（实验性）
- 🔧 **插件系统**：可扩展插件架构
...
### 渲染相关
- **表情转化**：表情过渡不够自然，部分表情可能无法正常显示在角色脸上
- **全身半身切换**：比例裁剪有小问题，大中小尺寸切换存在问题
- **嘴型同步**：音频振幅驱动代码已实现，但 Live2D 渲染器未接入 `set_lip_sync` 方法，当前不工作

### 记忆系统
- **架构优化中**：Mem0 集成已完成，但上下文注入、检索策略、分层管理仍需进一步优化
