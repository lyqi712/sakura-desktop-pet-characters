...
class CharacterVoice:
    ...
    ref_lang: str = "ja"
    text_lang: str = "ja"
    gpt_sovits_voice_params: dict[str, Any] = field(default_factory=dict)
    gpt_sovits_tone_params: dict[str, dict[str, Any]] = field(default_factory=dict)
...
class CharacterProfile:
    ...
    # 语言策略（原样保留，供外部工具/开发者校验角色的允许/禁止输出语言；
    # core 不做语义校验，语言限制的实际执行落在角色卡 system prompt 里）。
...
def _load_profile(manifest_path: Path) -> CharacterProfile:
    ...
    return CharacterProfile(
        ...
        language_policy=_load_language_policy(raw_data),
    )
...
def _load_language_policy(raw_data: dict[str, Any]) -> dict[str, Any] | None:
    """读取角色清单的 language_policy 段（原样保留，不做语义校验）。"""
    cfg = raw_data.get("language_policy")
    return cfg if isinstance(cfg, dict) else None
...
def _load_voice(package_dir: Path, voice_data: Any, manifest_path: Path) -> CharacterVoice | None:
    ...
    return CharacterVoice(
        ...
        gpt_sovits_voice_params=_load_gpt_sovits_voice_params(voice_data.get("gpt_sovits")),
        gpt_sovits_tone_params=_load_gpt_sovits_tone_params(voice_data.get("gpt_sovits")),
    )


def _load_gpt_sovits_voice_params(data: Any) -> dict[str, Any]:
...
def _load_gpt_sovits_tone_params(data: Any) -> dict[str, dict[str, Any]]:
    if not isinstance(data, dict):
        return {}
    ...
    for tone, params in tones.items():
        if not isinstance(tone, str) or not tone.strip() or not isinstance(params, dict):
            continue
        normalized[tone.strip()] = dict(params)
    return normalized
