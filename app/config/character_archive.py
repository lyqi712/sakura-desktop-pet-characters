...
def export_character_archive(profile: CharacterProfile, output_path: Path, *, include_voice: bool = True) -> None:
    ...
    if include_voice and profile.voice is not None:
        ...
        _copy_gpt_sovits_voice_config(profile.voice, character_manifest["voice"])

    renderer_manifest = _prepare_renderer_manifest_for_export(
        profile.renderer_config,
        package_dir=profile.package_dir,
        package_archive_names=package_archive_names,
        external_paths=external_paths,
    )
    if renderer_manifest is not None:
        character_manifest["renderer"] = renderer_manifest

    if isinstance(profile.language_policy, dict):
        character_manifest["language_policy"] = dict(profile.language_policy)
...
def export_character_voice_archive(profile: CharacterProfile, output_path: Path) -> None:
    ...
    _copy_gpt_sovits_voice_config(profile.voice, voice_manifest)
...
def _normalized_import_character_data(
    ...
    *,
    ...
    renderer_data = character_data.get("renderer")
    if renderer_data is not None:
        normalized["renderer"] = _normalized_renderer_for_import(package_dir, renderer_data)

    language_policy_data = character_data.get("language_policy")
    if isinstance(language_policy_data, dict):
        normalized["language_policy"] = dict(language_policy_data)
...
def _normalized_voice(voice_data: Any) -> dict[str, Any]:
    if not isinstance(voice_data, dict):
    ...
    gpt_sovits = _normalized_gpt_sovits_voice_config(voice_data.get("gpt_sovits"))
    if gpt_sovits:
        normalized["gpt_sovits"] = gpt_sovits
    return normalized


def _normalized_voice_archive(voice_data: Any) -> dict[str, Any]:
    if not isinstance(voice_data, dict):
...
def _normalized_gpt_sovits_voice_config(data: Any) -> dict[str, Any]:
    if not isinstance(data, dict):
        return {}
    normalized: dict[str, Any] = {}
    ...
    if isinstance(tones, dict):
        normalized_tones = {
            tone.strip(): dict(tone_params)
            for tone, tone_params in tones.items()
            if isinstance(tone, str) and tone.strip() and isinstance(tone_params, dict)
        }
        if normalized_tones:
            normalized["tones"] = normalized_tones
    return normalized
...
def _prepare_renderer_manifest_for_export(
    ...
    *,
    ...
    """把 renderer 段纳入归档：model_dir 指向包外目录时把模型文件一并打包。

    渐染器（Live2D/MMD 等）可能把模型目录配置为绝对路径或包外相对路径
    （见 plugins/live2d_renderer/plugin.py 的 model_dir 解析）。此前
    export_character_archive 完全不写 renderer 段，导致导出的 .char 包
    丢失渲染器配置，且包外模型文件也不会被打进 zip。
    """

    if not isinstance(renderer_config, dict):
        return None

    cfg = dict(renderer_config)
    model_dir_text = cfg.get("model_dir")
    if isinstance(model_dir_text, str) and model_dir_text.strip():
        model_dir_path = Path(model_dir_text.strip())
        if not model_dir_path.is_absolute():
            model_dir_path = (package_dir / model_dir_path).resolve()
        else:
            model_dir_path = model_dir_path.resolve()

        package_root = _resolved(package_dir)
        try:
            rel_to_package = model_dir_path.relative_to(package_root)
        except ValueError:
            rel_to_package = None

        if rel_to_package is not None:
            # 模型目录已在角色包内，会被 package_files 扫描自动收录，只需
            # 把 model_dir 归一化为包内相对路径（正斜杠）。
            cfg["model_dir"] = PurePosixPath(*rel_to_package.parts).as_posix() if rel_to_package.parts else "."
        else:
            if not model_dir_path.is_dir():
                raise CharacterArchiveError(f"渲染器模型目录不存在：{model_dir_path}")
            archive_model_dir = PurePosixPath(ARCHIVE_CHARACTER_ROOT.as_posix(), "renderer", "model_dir")
            for source in sorted(model_dir_path.rglob("*")):
                if not source.is_file():
                    continue
                rel = source.relative_to(model_dir_path)
                archive_path = PurePosixPath(archive_model_dir, *rel.parts)
                _safe_archive_path(archive_path.as_posix(), "renderer model_dir path")
                resolved_source = _resolved(source)
                if resolved_source not in external_paths:
                    external_paths[resolved_source] = archive_path
            # model_dir 归一化为包内相对路径，导入方解压后即可按同一结构还原。
            cfg["model_dir"] = "renderer/model_dir"

    return cfg


def _normalized_renderer_for_import(package_dir: Path, renderer_data: Any) -> dict[str, Any]:
    if not isinstance(renderer_data, dict):
        raise CharacterArchiveError("character.renderer 必须是对象。")

    cfg = dict(renderer_data)
    model_dir_text = cfg.get("model_dir")
    model_dir_path: Path | None = None
    if isinstance(model_dir_text, str) and model_dir_text.strip():
        rel = _safe_package_path(model_dir_text, "character.renderer.model_dir")
        model_dir_path = package_dir / rel
        if not model_dir_path.is_dir():
            raise CharacterArchiveError(f"渲染器模型目录不存在：{model_dir_path}")
        cfg["model_dir"] = rel.as_posix()

    model_text = cfg.get("model")
    if isinstance(model_text, str) and model_text.strip() and not _looks_like_renderer_url(model_text):
        base_dir = model_dir_path if model_dir_path is not None else package_dir
        model_rel = _safe_package_path(model_text, "character.renderer.model")
        if not (base_dir / model_rel).is_file():
            raise CharacterArchiveError(f"渲染器模型文件不存在：{base_dir / model_rel}")

    return cfg


def _looks_like_renderer_url(value: str) -> bool:
    lowered = value.strip().lower()
    return lowered.startswith(("http://", "https://", "qrc:", "data:"))
...
def _package_character_data(character_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    renderer_data = character_manifest.get("renderer")
    if isinstance(renderer_data, dict):
        package_data["renderer"] = dict(renderer_data)
    language_policy_data = character_manifest.get("language_policy")
    if isinstance(language_policy_data, dict):
        package_data["language_policy"] = dict(language_policy_data)
    return package_data


def _package_voice_data(voice_data: dict[str, Any]) -> dict[str, Any]:
    package_voice: dict[str, Any] = {
    ...
    gpt_sovits = _normalized_gpt_sovits_voice_config(voice_data.get("gpt_sovits"))
    if gpt_sovits:
        package_voice["gpt_sovits"] = gpt_sovits
    return package_voice


def _copy_gpt_sovits_voice_config(voice: object, target: dict[str, Any]) -> None:
    params = dict(getattr(voice, "gpt_sovits_voice_params", {}) or {})
    tone_params = {
        tone: dict(values)
        for tone, values in (getattr(voice, "gpt_sovits_tone_params", {}) or {}).items()
        if isinstance(tone, str) and isinstance(values, dict)
    }
    gpt_sovits: dict[str, Any] = {}
    if params:
        gpt_sovits["params"] = params
    if tone_params:
        gpt_sovits["tones"] = tone_params
    if gpt_sovits:
        target["gpt_sovits"] = gpt_sovits
