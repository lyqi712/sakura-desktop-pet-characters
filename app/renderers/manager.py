...
class RendererManager:
    ...
    @property
    def uses_owner_coordinates(self) -> bool:
        """当前后端的几何坐标是否相对于宿主窗口。"""
        return bool(getattr(self._active, "uses_owner_coordinates", False))

    @property
    def supports_drag_snapshot(self) -> bool:
        """当前后端是否支持拖拽时切换到宿主窗口快照层。"""
        return bool(getattr(self._active, "supports_drag_snapshot", False))
    ...
    def capture_snapshot(self) -> Any | None:
        ...
        def capture() -> None:
            nonlocal result
            capture_snapshot = getattr(self._active, "capture_snapshot", None)
            if callable(capture_snapshot):
                result = capture_snapshot()

        self._safe("capture_snapshot", capture)
        return result

    def set_geometry(self, x: int, y: int, width: int, height: int, layout_height: int | None = None) -> None:
        self._safe("set_geometry", lambda: self._active.set_geometry(x, y, width, height, layout_height))
    ...
    def set_clip_mask(self, keep_top_height: int | None) -> None:
        self._safe("set_clip_mask", lambda: self._active.set_clip_mask(keep_top_height))

    def set_scale(self, scale: float) -> None:
        self._safe("set_scale", lambda: self._active.set_scale(scale))

    def set_mode(self, mode: str) -> None:
        self._safe("set_mode", lambda: self._active.set_mode(mode))
    ...
    def trigger_scene_action(self, kind: str, name: str | None = None) -> None:
        self._safe("trigger_scene_action", lambda: self._active.trigger_scene_action(kind, name))
