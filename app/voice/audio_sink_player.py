...
# 播放前补一小段静音，规避部分 Windows/Qt 音频后端启动瞬间吃掉首帧的问题。
_SINK_LEADING_SILENCE_MS = 220
# 播放末尾补静音，避免 QAudioSink stop/设备排空时截掉最后 1-2 个字。
_SINK_TRAILING_SILENCE_MS = 500
...
class AudioSinkPlayer(QObject):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self._leading_silence_bytes: int = 0
        self._trailing_silence_bytes: int = 0
        self._started_threshold_bytes: int = 0
    ...
    def start(self, audio_path: Path) -> bool:
        ...
        self._leading_silence_bytes = 0
        self._trailing_silence_bytes = 0
        self._started_threshold_bytes = 0
        self._write_count = 0
        ...
        if self._bytes_per_second > 0 and (_SINK_LEADING_SILENCE_MS > 0 or _SINK_TRAILING_SILENCE_MS > 0):
            frame_bytes = self._channels * self._sample_width
            leading_bytes = int(self._sample_rate * _SINK_LEADING_SILENCE_MS / 1000) * frame_bytes
            leading_bytes -= leading_bytes % frame_bytes
            trailing_bytes = int(self._sample_rate * _SINK_TRAILING_SILENCE_MS / 1000) * frame_bytes
            trailing_bytes -= trailing_bytes % frame_bytes
            if leading_bytes > 0 or trailing_bytes > 0:
                self._pcm_buffer = (b"\x00" * leading_bytes) + self._pcm_buffer + (b"\x00" * trailing_bytes)
                self._total_pcm_bytes = len(self._pcm_buffer)
                self._leading_silence_bytes = leading_bytes
                self._trailing_silence_bytes = trailing_bytes
                self._duration_ms += _SINK_LEADING_SILENCE_MS + _SINK_TRAILING_SILENCE_MS
                # started 回调延迟到前置静音 + 额外 100ms 缓冲写入后，给设备更多预热时间
                self._started_threshold_bytes = leading_bytes + int(self._sample_rate * 0.1) * frame_bytes
        ...
        debug_log(
            ...
            "AudioSink: 开始播放",
            {
                ...
                "leading_silence_ms": _SINK_LEADING_SILENCE_MS,
                "trailing_silence_ms": _SINK_TRAILING_SILENCE_MS,
            },
        )
        ...
        try:
            ...
            if chunk_size > 0:
                ...
                if self._io_device is not None and hasattr(self._io_device, "write"):
                    ...
                    written = 0
                if written < 0:
                    raise OSError("QAudioSink IO device write returned a negative byte count")
                self._write_offset += written
                self._write_count += 1
                if (
                    written > 0
                    and not self._started_emitted
                    and self._write_offset > self._started_threshold_bytes
                ):
