...
SEGMENTED_REPLY_FORMAT = (
    '{"segments":[{"ja":"角色原文","zh":"中文译文或同文","tone":"中性","portrait":"站立待机"}]}'
)

AGENT_REPLY_FORMAT = """{
  "segments": [
    {"ja": "角色原文", "zh": "中文译文或同文", "tone": "中性", "portrait": "站立待机"}
  ]
}"""
    ...
    return PromptBlock(
        ...
        "\n".join(
            [
                ...
                "- 不要机械使用中性；只要语义里有撒娇、害羞、请求、困惑、惊讶、不满等情绪，就必须选对应 tone，让 Live2D 表情跟随变化。",
...
def translation_rules_block() -> PromptBlock:
    return PromptBlock(
        ...
        "\n".join(
            [
                "- ja 填角色说话的原文供 TTS 朗读：角色是中文角色则填中文，日文角色则填自然日语；不要填与角色语言不符的内容。",
                "- zh 填中文译文；若 ja 已是中文，zh 可省略或与 ja 保持一致。",
                "- 输出前静默自检每个 ja，确保语言符合角色设定；不输出自检过程。",
                "- 例（日文角色）：ja=\"原因は Mermaid の構文みたい。\"，zh=\"原因像是 Mermaid 语法。\"。",
                "- 例（中文角色）：ja=\"原来如此。\"，zh 可同文。",
                "- zh 只写 ja 的对应中文；ja/zh 一一对应，不加解释、动作旁白或标签。",
            ]
        ),
    )
...
def proactive_scene_strategy_block() -> PromptBlock:
    return PromptBlock(
        ...
        "\n".join(
            [
                "- 代码/调试/报错：点出文件、函数、错误或修改点，再轻问是否卡住。",
                "- 文档/学习/资料：点出标题、主题或段落，帮用户整理。",
                "- 图片/角色/女性照片：无严肃任务上下文时可轻微吃醋或傲娇；不要指责。",
                "- 视频/漫画/游戏：轻松陪聊，别立刻泛化提醒。",
