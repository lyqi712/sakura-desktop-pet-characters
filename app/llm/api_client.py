...
class OpenAICompatibleClient:
    def __init__(self, settings: ApiSettings) -> None:
        ...
        self._request_semaphore = _build_request_semaphore(settings.max_concurrent_requests)
    ...
    def _post_chat_completions_with_compatibility_fallbacks(
        ...
        *,
        ...
        if semaphore is None:
            return self._post_chat_completions_with_compatibility_fallbacks_unlocked(
                payload,
                cancel_checker=cancel_checker,
            )
        _acquire_request_slot(semaphore, cancel_checker)
        try:
            return self._post_chat_completions_with_compatibility_fallbacks_unlocked(
                payload,
                cancel_checker=cancel_checker,
    ...
    def _post_chat_completions_with_compatibility_fallbacks_unlocked(
        self,
        payload: dict[str, Any],
        *,
        cancel_checker: CancelChecker | None = None,
        ...
        *,
        ...
        try:
            ...
            if isinstance(data, dict) and isinstance(data.get("error"), dict):
                raise ApiRequestError(_format_api_error_object(data["error"], url))
...
def _format_api_error_object(error: dict[str, Any], url: str) -> str:
    """把 OpenAI 兼容接口返回的 error 对象转成可读诊断。"""
    message = str(error.get("message") or error.get("error") or "").strip()
    error_type = str(error.get("type") or "").strip()
    code = str(error.get("code") or "").strip()
    lower = message.lower()
    if "trial cost limit" in lower or "upgrade your plan" in lower or "402" in lower:
        return (
            "API 上游额度已用尽或套餐受限："
            f"{message or json.dumps(error, ensure_ascii=False)}"
            "\n请更换可用 API Key/供应商，或升级当前供应商套餐后重试。"
        )
    parts = ["API 上游返回错误"]
    if error_type:
...
def _build_request_semaphore(limit: int | None) -> threading.BoundedSemaphore | None:
    try:
